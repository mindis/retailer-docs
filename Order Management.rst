...
.. csv-table::  
   ...
   :widths: 20,80   
    ...
    "At Risk", "Some order are temporarily 'At Risk' while Rangespan escalates any supply issues with our vendors."
    "Shipped", "Order has been shipped. Depending on the shipping method, it can be 1-3 days before the customer receives the item. If the supplier uses a tracked delivery method (higher value orders) carrier tracking details are made available from the time an order enters this state."
    ...
    "Refunded", "Usually, this corresponds to a customer returning the goods within the agreed terms. An order that is refunded will show up as 2 lines in the invoice: once on the sale, and once for the refund deduction."
